...
public sealed class UnitOfWorkEF<TContext> : IRepositoryEFFactory, IUnitOfWorkEF<TContext>
    where TContext : DbContext
{
    ...
    public IRepositoryEF<TEntity> GetRepository<TEntity>(bool hasCustomRepository = false) where TEntity : class
    {
        ...
        {
            var customRepo = DbContext.GetService<IRepositoryEF<TEntity>>();
            if (customRepo != null)
            {
                return customRepo;
            }
        ...
        return (IRepositoryEF<TEntity>)_repositories[type];
    }
    ...
    public async Task<int> SaveChangesAsync(params IUnitOfWorkEF[] unitOfWorks)
    {
        var count = 0;
